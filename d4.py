...
class agent():
	def __init__(self):
		...
		self.drone_position_4=[0.0,0.0,0.0]
	...
	def get_d(self):
		self.rel_d[0]=math.sqrt((self.drone_position_2[0]-self.drone_position_3[0])**2+(self.drone_position_2[1]-self.drone_position_3[1])**2+(self.drone_position_2[2]-self.drone_position_3[2])**2)
		self.rel_d[1]=math.sqrt((self.drone_position_2[0]-self.drone_position_4[0])**2+(self.drone_position_2[1]-self.drone_position_4[1])**2+(self.drone_position_2[2]-self.drone_position_4[2])**2)
		self.rel_d[2]=math.sqrt((self.drone_position_3[0]-self.drone_position_4[0])**2+(self.drone_position_3[1]-self.drone_position_4[1])**2+(self.drone_position_3[2]-self.drone_position_4[2])**2)
		
	def getAngle(self,A):
       		B=self.drone_position_2
	...
	def calc_setpoint(self):
		self.setpoint_1[0]=(self.drone_position_2[0]+self.drone_position_3[0]+self.drone_position_4[0])/3.0
		self.setpoint_1[1]=(self.drone_position_2[1]+self.drone_position_3[1]+self.drone_position_4[1])/3.0
		self.setpoint_1[2]=(self.drone_position_2[2]+self.drone_position_3[2]+self.drone_position_4[2])/3.0
		
	def pid(self,action_index):
		self.error_1 = [dp - setp for dp,setp in zip(self.drone_position_4, self.setpoint_1)]
...
def initial_step(drone,initial_setpoint):
		drone.error_1 = [dp - setp for dp,setp in zip(drone.drone_position_4, initial_setpoint)]
...
if ('__main__'==__name__):
	...
	Q=Q()
	print(Q.q_table)
	initial_setpoint=[drone.drone_position_4[0],drone.drone_position_4[1],5.0]
	
	'''
	Initial hovering to required plane
	'''
	while(not(4.7< drone.drone_position_4[2] <5.10)):
		
		initial_step(drone,initial_setpoint)
		time.sleep(drone.sample_time)
	print(drone.drone_position_4,' reached the required plane')
	drone.calc_setpoint()
	drone.pid_reset()
	present_angle=drone.getAngle(drone.drone_position_4)
	...
	while not rospy.is_shutdown():
		if(drone.check == 2):		# System formed, proceed to new location in same orientation
			...
			new_cen_coo=[8.3,12,5]   # New location coordinates
			drone.calc_setpoint
			...
			new_setpoint = [a + b for a, b in zip(drone.drone_position_4, delta)]
			while(True):
		...
		if(not(c) and not ((57<=drone.alpha<=63) and (57<=drone.betta<=63) and (57<=drone.gamma<=63) and(drone.rel_d[0]<8) and(drone.rel_d[1]<8) and(drone.rel_d[2]<8))):
			...
			while(True):
				...
				cur_pos=drone.drone_position_4

...
class agent():
	...
	def pid(self,action_index):
		self.error_1 = [dp - setp for dp,setp in zip(self.drone_position_3, self.setpoint_1)]
...
def initial_step(drone,initial_setpoint):
		drone.error_1 = [dp - setp for dp,setp in zip(drone.drone_position_3, initial_setpoint)]
...
if ('__main__'==__name__):
	...
	Q=Q()
	print(Q.q_table)
	initial_setpoint=[drone.drone_position_3[0],drone.drone_position_3[1],5.0]
	
	'''
	Initial hovering to required plane
	'''
	while(not(4.7< drone.drone_position_3[2] <5.10)):
		
		initial_step(drone,initial_setpoint)
		time.sleep(drone.sample_time)
	print(drone.drone_position_3,' reached the required plane')
	drone.calc_setpoint()
	drone.pid_reset()
	present_angle=drone.getAngle(drone.drone_position_3)
	...
	while not rospy.is_shutdown():
		if(drone.check == 2):		# System formed, proceed to new location in same orientation
			...
			new_setpoint = [a + b for a, b in zip(drone.drone_position_3, delta)]
			while(True):
		...
		if(not(c) and not ((57<=drone.alpha<=63) and (57<=drone.betta<=63) and (57<=drone.gamma<=63) and(drone.rel_d[0]<8) and(drone.rel_d[1]<8) and(drone.rel_d[2]<8))):
			...
			while(True):
				...
				cur_pos=drone.drone_position_3
